          ...
          {!selectedProject ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
              {projects.map((project) => (
                ...
                >
                  <div style={{ height: '150px', background: '#eee', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '2px solid #000', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '15px' }}>
                    <h3 style={{ fontFamily: '"Press Start 2P", cursive', fontSize: '12px', marginTop: 0 }}>{project.title}</h3>
                    <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '14px', color: '#555' }}>{project.description}</p>
                    <div style={{ marginTop: '10px' }}>
                        <span style={{ fontSize: '12px', fontWeight: 'bold' }}>ROLE: </span>
                        <span style={{ fontSize: '12px' }}>{project.role}</span>
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div>
                ...
                <div style={{ marginBottom: '40px', borderBottom: '2px solid #eee', paddingBottom: '20px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px', flexWrap: 'wrap', gap: '20px' }}>
                      <div>
                        <h1 style={{ fontFamily: 'Inter, sans-serif', fontWeight: 800, fontSize: '32px', margin: 0 }}>{selectedProject.title}</h1>
                        <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '18px', color: '#666' }}>{selectedProject.description}</p>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                          <div style={{ marginBottom: '5px' }}><strong>Role:</strong> {selectedProject.role}</div>
                          <div style={{ marginBottom: '5px' }}><strong>Timeline:</strong> {selectedProject.timeline}</div>
                          <div>
                            {selectedProject.techStack.map(tech => (
                                <span key={tech} style={{ background: '#eee', padding: '4px 8px', borderRadius: '4px', fontSize: '12px', marginRight: '5px', display: 'inline-block' }}>{tech}</span>
                            ))}
                          </div>
                      </div>
                  </div>
                ...
                <section style={{ marginBottom: '40px' }}>
                    ...
                    <div style={{ background: '#f9f9f9', padding: '20px', borderRadius: '8px', borderLeft: '4px solid #F39C12', marginTop: '15px' }}>
                        <p style={{ fontFamily: 'Inter, sans-serif', lineHeight: 1.6 }}><strong>Context:</strong> {selectedProject.problemStatement.context}</p>
                        <p style={{ fontFamily: 'Inter, sans-serif', lineHeight: 1.6 }}><strong>Constraints:</strong> {selectedProject.problemStatement.constraints}</p>
                        <ul style={{ fontFamily: 'Inter, sans-serif', lineHeight: 1.6 }}>
                ...
                <section style={{ marginBottom: '40px' }}>
                    ...
                    <p style={{ fontFamily: 'Inter, sans-serif', lineHeight: 1.6, marginTop: '15px' }}>{selectedProject.research.summary}</p>
                        ...
                        <div style={{ background: '#eef', padding: '15px', borderRadius: '8px' }}>
                            ...
                            <ul style={{ paddingLeft: '20px' }}>
                                {selectedProject.research.insights.map((insight, i) => (
                                    <li key={i} style={{ fontFamily: 'Inter, sans-serif', marginBottom: '8px' }}>{insight}</li>
                                ))}
                            </ul>
                        </div>
                        <div>
                ...
                <section style={{ marginBottom: '40px' }}>
                    ...
                    <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: '18px', marginTop: '20px' }}>User Flows</h3>
                    ...
                    <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: '18px', marginTop: '20px' }}>Wireframes & Mockups</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '15px' }}>
                         {selectedProject.designProcess.wireframes.map((img, i) => (
                             <div key={i}>
                                 ...
                                 <p style={{ textAlign: 'center', fontSize: '12px', color: '#666' }}>Low-Fidelity</p>
                             </div>
                         ))}
                         {selectedProject.designProcess.mockups.map((img, i) => (
                             <div key={i}>
                                 ...
                                 <p style={{ textAlign: 'center', fontSize: '12px', color: '#666' }}>High-Fidelity</p>
                             </div>
                         ))}
                    </div>
                    ...
                    {selectedProject.designProcess.prototypeUrl && (
                        <div style={{ marginTop: '30px' }}>
                             <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: '18px' }}>Interactive Prototype</h3>
                     ...
                     <div style={{ display: 'flex', gap: '40px', marginTop: '20px', flexWrap: 'wrap' }}>
                        {selectedProject.outcomes.metrics.map((metric, i) => (
                            <div key={i} style={{ flex: 1, minWidth: '200px' }}>
                                <h3 style={{ fontSize: '24px', color: '#2ECC71', margin: '0 0 10px 0' }}>{metric}</h3>
                            </div>
                        ))}
                     </div>
                     <p style={{ fontFamily: 'Inter, sans-serif', lineHeight: 1.6, marginTop: '20px', borderTop: '1px solid #555', paddingTop: '20px' }}>
